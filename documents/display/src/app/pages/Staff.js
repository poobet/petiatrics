...
import { Search, Filter, Plus, MoreVertical, Mail, Phone, Calendar, Clock, Edit, Trash2, Eye, UserCheck, } from 'lucide-react';
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="flex-1 relative">
          ...
          <input type="text" placeholder="Search staff by name, role, or email..." className="w-full pl-10 pr-4 py-2.5 bg-white border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"/>
        </div>
        <DropdownMenu>
          ...
          <DropdownMenuContent>
            {roles.map((role) => (<DropdownMenuItem key={role} onClick={() => setFilterRole(role)}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredStaff.map((staff) => {
            ...
            return (<Card key={staff.id} className="hover:shadow-lg transition-shadow">
              <CardContent className="pt-6">
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            const todayAppointments = mockAppointments.filter(apt => apt.veterinarian === staff.name && apt.date === '2026-02-18');
            return (<div key={staff.id} className="flex items-center gap-4 p-4 bg-gray-50 rounded-lg">
