...
import { Search, Filter, Plus, MoreVertical, Download, Eye, Edit, Trash2, ArrowUpDown, Calendar, } from 'lucide-react';
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="flex-1 relative">
          ...
          <input type="text" placeholder="Search patients by name, breed, or owner..." className="w-full pl-10 pr-4 py-2.5 bg-white border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"/>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200">
                {filteredPets.map((pet) => (<tr key={pet.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => setSelectedPet(pet)}>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          {pet.microchipId && (<p className="text-xs text-gray-500">ID: {pet.microchipId}</p>)}
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
              ...
              <Tabs defaultValue="info" className="mt-4">
                ...
                <TabsContent value="medical" className="mt-4">
                  <div className="space-y-4">
                    {petMedicalRecords.length > 0 ? (petMedicalRecords.map((record) => (<Card key={record.id}>
                          <CardContent className="pt-6">
                            <div className="flex items-start justify-between">
                              <div className="space-y-2">
                                ...
                                {record.prescription && (<p className="text-sm text-blue-600">Rx: {record.prescription}</p>)}
                                <p className="text-xs text-gray-500">By {record.veterinarian}</p>
                              </div>
                            </div>
                          </CardContent>
                        </Card>))) : (<p className="text-center text-gray-500 py-8">No medical records found</p>)}
                  </div>
                </TabsContent>

                <TabsContent value="appointments" className="mt-4">
                  <div className="space-y-3">
                    {petAppointments.length > 0 ? (petAppointments.map((apt) => (<Card key={apt.id}>
                          <CardContent className="pt-6">
