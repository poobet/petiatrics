...
type Screen = 'home' | 'pets' | 'pet-detail' | 'appointments' | 'book-appointment' | 'medical-records' | 'notifications' | 'profile';
...
export function MobileOwnerApp() {
  ...
  const upcomingAppointments = mockAppointments.filter(apt => 
    ownerPets.some(pet => pet.id === apt.petId) && 
    new Date(apt.date) >= new Date('2026-02-18')
  ).slice(0, 3);
  ...
  const navItems = [
    ...
    { id: 'pets' as Screen, icon: PawPrint, label: 'Pets' },
  ...
  const renderHeader = () => (
    <div className="sticky top-0 z-10 bg-white border-b border-gray-200 px-4 py-4">
      <div className="flex items-center justify-between">
        {currentScreen !== 'home' && currentScreen !== 'pets' && currentScreen !== 'appointments' && currentScreen !== 'profile' ? (
          <button onClick={() => {
            if (currentScreen === 'pet-detail') setCurrentScreen('pets');
            else if (currentScreen === 'book-appointment') setCurrentScreen('appointments');
    ...
    <div className="space-y-6 pb-20">
      ...
      <div className="px-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <button
            onClick={() => setCurrentScreen('book-appointment')}
    ...
    return (
      <div className="space-y-4 pb-20 p-4">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <Badge className={`mt-2 ${
                  selectedPet.status === 'healthy' ? 'bg-green-100 text-green-700 hover:bg-green-100' :
        ...
        <div>
          ...
          <div className="space-y-3">
            {petRecords.length > 0 ? (
              ...
              <p className="text-center text-gray-500 py-8 text-sm">No medical records yet</p>
            )}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-3">
            {petAppointments.slice(0, 3).map((apt) => (
              <Card key={apt.id}>
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
            ...
            <CardContent className="p-4">
              <div className="flex items-start gap-3">
                <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${
                  apt.type === 'emergency' ? 'bg-orange-100' :
                  apt.type === 'surgery' ? 'bg-blue-100' :
                  apt.type === 'grooming' ? 'bg-green-100' :
                  'bg-gray-100'
                }`}>
                  <Calendar className={`w-6 h-6 ${
                    apt.type === 'emergency' ? 'text-orange-600' :
      ...
      <Card>
        <CardContent className="p-4 space-y-4">
          ...
          <div>
            ...
            <textarea 
              className="w-full mt-2 px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg resize-none"
              rows={3}
    ...
    <div className="space-y-3 pb-20 p-4">
      ...
      <div className="space-y-2">
        {notifications.map((notif) => (
          <Card key={notif.id} className={!notif.read ? 'border-l-4 border-l-blue-600' : ''}>
            <CardContent className="p-4">
              <div className="flex items-start gap-3">
