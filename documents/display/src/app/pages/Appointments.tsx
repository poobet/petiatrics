            ...
            <CardContent className="p-6">
              ...
              <div className="grid grid-cols-8 gap-px bg-gray-200 border border-gray-200 rounded-lg overflow-hidden">
                ...
                {currentWeek.map((day) => (
                  <div
                    key={day.date}
                    className={`bg-white p-3 ${day.date === 18 ? 'bg-blue-50' : ''}`}
                  >
                  ...
                  <Fragment key={`row-${time}`}>
                    ...
                    {currentWeek.map((day) => {
                      ...
                      const appointment = mockAppointments.find(
                        apt => apt.date === dayDate && apt.time.startsWith(time.split(':')[0])
                      );

                      return (
                        <div
                          key={`${day.date}-${time}`}
                          className={`bg-white p-2 border-t border-gray-100 min-h-[80px] ${
                            day.date === 18 ? 'bg-blue-50/50' : ''
                          }`}
                        >
                          {appointment && (
                            <div
                              className={`${getAppointmentColor(appointment.type)} text-white p-2 rounded-lg text-xs border-l-4 shadow-sm hover:shadow-md transition-shadow cursor-pointer`}
                            >
